...
- **`handle_disconnected_grid`** (contingency analysis only — **not** `ScenarioSweepGPU`, see the top-level workload list's deferred-scope note): the batch shares one fixed symbolic structure, so masking a contingency-islanded bus is value-only — identity its J row(s), zero its F row(s), report its V as NaN — solved on the largest connected component instead of skipping the whole scenario. Contingencies that strand the angle reference or a controller (HVDC/SVC/remote-gen) bus are still conservatively NaN-skipped. `optimize_reference_slack(grid, contingency_branch_ids)` (top-level export) re-picks and re-solves the base case with the reference slack least likely to be stranded, minimizing those skips. `ScenarioSweepGPU` only has the cheaper legacy `check_connectivity` skip-if-split path (a topology change that islands the grid drops that scenario, NaN) — porting the largest-component masking to `ScenarioSweepBatch` is future work.

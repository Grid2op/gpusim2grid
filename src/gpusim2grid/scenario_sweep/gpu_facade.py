...
class ScenarioSweepGPU:
    ...
    def set_gen_v(self, gen_v):
        ...
        generator's regulated bus before that scenario's solve, row-aligned
        with :meth:`set_injections`/:meth:`set_topology`. A generator that
        never regulates voltage, or is disconnected, has its column ignored.
        Requires a lightsim2grid grid (bridge mode) — raises
        ``RuntimeError`` in explicit-array (tuple) mode.

        Parameters
        ----------
        gen_v : (n_steps, n_gens) — target vm_pu per generator. Row count
            must match :meth:`set_injections`.
        """
        if self._elements is None:
            raise RuntimeError(

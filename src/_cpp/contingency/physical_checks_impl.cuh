...
// =============================================================================
// contingency/physical_checks_impl.cuh — session-side glue of the two post-
...
struct SetupTimes { double bus_q_ms = 0.; double hvdc_p_ms = 0.; };

// Before solve(): hand the driver what the two checks need and (re)seed their
// outputs, then produce the base ("n") case's own report when the base solve
// converged. Re-callable on a live driver (ScenarioSweep's persistent one):
// that is what resets the -1 "never simulated" sentinels between runs.
...
{
    tm.t_physical_setup_ms += t.bus_q_ms + t.hvdc_p_ms;

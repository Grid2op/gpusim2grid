...
GPU-accelerated AC power flow for electrical grids. The Python package is `gpusim2grid`; the compiled CUDA extension is `gpusim2grid._gpusim2grid`. Four workloads, all built on a single-system Newton-Raphson (NR) core:
...
- **Scenario sweep** (`ScenarioSweepGPU`) — row-aligned combination of the two: row `i`'s (P, Q) profile is solved together with row `i`'s own set of tripped branches, independently of every other row. Mirrors lightsim2grid's own `ScenarioSweep` (its `BaseBatchSweep<YbusPolicy, SbusPolicy, BatchInitKind>` policy refactor); deliberately a separate class from `InjectionSweepGPU`/`ContingencyAnalysisGPU` rather than an extension of either, since the usage pattern (one topology+injection pair per row) differs from both (a shared base case with a distinct scenario set). Supports both `handle_disconnected_grid` masking and `compute_limit_violations` reporting, identically to `ContingencyAnalysisGPU` (see those sections below) — the `ScenarioSweepBatch` source implements the real `fill_mask_buffers`/`tripped_branch_table`, not the no-op stubs an earlier version shipped with.
...
When solving through the lightsim2grid bridge (the default `grid=<solved LSGrid>` path on all four facades), gpusim2grid does not solve the bare `[pvpq | pq]` system — it reproduces the **same augmented NR system lightsim2grid poses**, including in-Jacobian power-system controls: distributed slack (`MultiSlack`), HVDC angle-droop, and SVC / remote generator voltage control (`VoltageControl`). Results match lightsim2grid bit-for-bit (to ~1e-15) on both the single solve and the batched contingency/injection-sweep/scenario-sweep paths.
...
- **Structural supersets** (`ledger_extend.{hpp,cpp}`): the CA/SS sessions may solve a J whose pattern is a strict *superset* of lightsim2grid's — every extra entry is filled per slot by value, so one cuDSS analysis still serves the batch. Two extensions exist: `reserve_stranded_controller_slots` (one `(v_row, q_col)` zero per lone GEN controller, see `handle_disconnected_grid`) and `add_switchable_vm_buses` (one Vm column + Q equation per bus, with the full dS pattern derived from Ybus, **appended at the end** of the ledger so no existing index moves — which is why the VoltageControl custom rows are first frozen into explicit `LedgerData::vc_v_rows`/`vc_share_rows` by `materialize_vc_custom_rows`; an empty `vc_v_rows` still means lightsim2grid's own "last `n_controllers` rows" layout). `AcPfGPU`/`InjectionSweepGPU` ledgers are never extended. `drop_multislack_augmentation` must run *before* either extension.
...
- **`handle_disconnected_grid`** (`ContingencyAnalysisGPU` and `ScenarioSweepGPU`; not `InjectionSweepGPU`, whose topology never changes): the batch shares one fixed symbolic structure, so masking a contingency-islanded bus is value-only — identity its J row(s), zero its F row(s), report its V as NaN — solved on the largest connected component instead of skipping the whole scenario. Contingencies/scenarios that strand the angle reference, an HVDC end, a *regulated* bus, or *every* controller of a VoltageControl group are still conservatively NaN-skipped. A stranded **lone** remote controller (a group of exactly one: a remotely-regulating generator or a voltage-mode SVC whose own bus is islanded while its regulated bus stays live) is recovered instead — lightsim2grid PR #192 parity: its bordered voltage row is repurposed by value into `Q_c == 0` (`J[v_row, q_col] = 1`, `J[v_row, vm_col(reg)] = 0`, `F[v_row] = -Q_c`) and the regulated bus floats as an ordinary PQ bus, matching a one-off solve with that controller deactivated. The `(v_row, q_col)` slot a lone GEN controller needs is a structural zero the CA/SS bridge factories always reserve (`reserve_stranded_controller_slots`, `ledger_extend.{hpp,cpp}`; an SVC's slot already carries its slope). Per-slot machinery: `Contingency::stranded_groups` (decided in `compute_component_masks` from `MaskConfig`'s VC group topology), the `jov`/`str` streams of `MaskEntries`/`MaskStreams` (`contingency/mask_streams.cuh`, shared by both batch sources), and `apply_J_overrides_kernel` / `vc_stranded_vrow_kernel` launched by the `nr_apply_J_masks` / `nr_apply_F_masks` composites (`nr_iter_step.cuh`) — always *after* the feature stamps and *before* the bus mask. A stranded member of a multi-controller group is left alone (its sharing rows keep the column coupled). `optimize_reference_slack(grid, contingency_branch_ids)` (top-level export) re-picks and re-solves the base case with the reference slack least likely to be stranded, minimizing those skips (contingency analysis only). Both sessions build their `MaskConfig` through the shared `build_mask_config` (`mask_config_builder.cuh`), and `ScenarioSweepBatch` reuses `compute_component_masks`/`build_mask_entries`/`MaskStreams` verbatim from the contingency stack.
...
- **Branch admittances are always the Kron-reduced *effective* ones, and are named `*_eff`**: `yff_eff`/`yft_eff`/`ytf_eff`/`ytt_eff` (`h_*_eff_`, `d_*_eff` on the host/device side) are lightsim2grid's `yac_eff_*`, never the raw `yac_*`. They differ only for a *half-open* branch (`init_from_pypowsybl(..., keep_half_open_lines=True)`: connected on one side, the open end Kron-reduced out and relabeled to bus `-1`), where the raw value at the energized bus is the full series admittance while the effective one is just what survives the reduction (line charging; exactly 0 for an ideal trafo). Patching Ybus with the raw value on a trip would remove an admittance the base case never had, so `build_contingency_from_branch_ids` also skips triplets touching a `-1` endpoint. `tests/python/test_half_open_lines.py` pins this against `ContingencyAnalysisCPP`.

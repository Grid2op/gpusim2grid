...
iterations. Needs more NR iterations than direct_refactor_every.

Usage (from gpusim2grid/benchmarks/):
    python run_iter0_only_sweep.py
    python run_iter0_only_sweep.py --output results_iter0.csv
"""
...
# More iterations needed: Jacobian is updated only at iteration 0
NB_ITERS = [5, 10, 15]
MAX_CONT_CPU = 500
MAX_CONT_GPU = 10_000
...
def main():
    parser = argparse.ArgumentParser(
        description="Sweep runner for the direct_iter0_only strategy"
    )
    parser.add_argument("--nb_iters", nargs="+", type=int, default=NB_ITERS,
                        help="NR iteration counts to sweep (default: 3 5 8 10 15)")
    parser.add_argument("--output", default="sweep_results_iter0_only.csv",
                        help="Output CSV file (default: sweep_results_iter0_only.csv)")
